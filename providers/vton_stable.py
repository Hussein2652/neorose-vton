...
class StableVITONClient:
    ...
    def process(self, user_image_path: str, garment_image_path: str, out_dir: str, user_mask_path: Optional[str] = None) -> Optional[str]:
        ...
        try:
            files = {}
            with open(user_image_path, 'rb') as fu:
                files['user'] = (os.path.basename(user_image_path), fu, 'image/jpeg')
                with open(garment_image_path, 'rb') as fg:
                    files['garment'] = (os.path.basename(garment_image_path), fg, 'image/png')
